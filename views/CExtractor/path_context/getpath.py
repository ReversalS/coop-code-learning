...
class Method:
    ...
    def print_path(self):
        return ' '.join([Method._print_path(p) for p in self.contexts])
...
def get_program_paths(j, max_path_length=None, max_path_width=None):
    visited = [False] * len(j)
    ...
    for i, node in enumerate(j):
        if not visited[i] and node["type"] == "FuncDef":
            bag_of_paths, _ = traverse(i)
            method = Method(node['value'] if hasattr(node,'value') else '', bag_of_paths)
            global_paths.append(method.print_path())
    return ' '.join(global_paths)
    # bag_of_paths, _ = traverse(0)
    # return Method('dummy-method', bag_of_paths)
