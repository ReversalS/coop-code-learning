...
class Pipeline:
    ...
    # parse source code
    def parse_source(self, output_file, cached=False):
        ...
        if os.path.exists(path) and cached:
            ...
            source = pd.read_pickle(self.root+'source_df.pkl')

            # source.columns = ['id', 'code', 'label']
            # source['code'] = source['code'].apply(parser.parse)
            source['source'] = source['source'].apply(parser.parse)
    ...
    def dictionary_and_embedding(self, input_file, size):
        ...
        def trans_to_sequences(ast):
            ...
            return sequence
        print(trees.columns)
        corpus = trees['source'].apply(trans_to_sequences)
    ...
    def generate_block_seqs(self,data_path,part):
        ...
        trees['source'] = trees['source'].apply(trans2seq)
        trees.to_pickle(self.root+part+'/blocks.pkl')

    # run for processing data to train
    def run(self):
        print('parse source code...')
        # self.parse_source(output_file='ast.pkl', cached=True)
        print('split data...')
...
ppl = Pipeline('3:1:1', 'data/poj_cls/')
ppl.run()
