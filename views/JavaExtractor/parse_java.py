from views.JavaExtractor import javalang
from views.PythonExtractor.parse_python import read_file_to_string
import json as json

def parse_file(filename):
    global c, d
    tree = javalang.parse.parse(read_file_to_string(filename))
    ...
    def traverse(node, node_type=None):
        ...
        if isinstance(node, list):
            ...
            for k, v in zip(node.attrs, node.children):
                traverse_child(k, v)
